...
from amazon.pipelines import KinesisPipeline

class ReviewsSpider(scrapy.Spider):
	name = 'reviews'

	def start_requests(self):
		urltoreview = [scrapy.FormRequest('http://www.amazon.com/hz/reviews-render/ajax/reviews/get/ref=cm_cr_othr_d_paging_btm_next_2/',
			formdata={
			'sortBy': '',
	...
	def parse(self, response):
		...
		for review in reviews:
			...
			yield il.load_item()
			pipeline = KinesisPipeline()
			pipeline.putIn(il.load_item())